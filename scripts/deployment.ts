...
import tokenAbi from "../build/tokenlockmap-abi.json";
...
async function main() {
  ...
  const contractAccount = Signer.fromWif(
    network.accounts.tokenlockMapMapContract.privateKey
  );
  ...
  const contract = new Contract({
    ...
    bytecode: fs.readFileSync(
      path.join(__dirname, "../build/release/tokenlockmap.wasm")
    ),
    options: {
